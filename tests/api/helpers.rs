...
use email_newsletter::startup::{Application, build_connection_pool};
...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    if std::env::var("TEST_LOG").is_ok() {
        let subscriber = get_subsciber(subscriber_name, default_filter_level, std::io::stdout);
        init_subscriber(subscriber);
    } else {
        let subscriber = get_subsciber(subscriber_name, default_filter_level, std::io::sink);
        init_subscriber(subscriber);
    }
});
...
impl TestApp {
    ...
    pub fn get_confirmation_links(&self, email_request: &wiremock::Request) -> ConfirmationLinks {
        ...
        let get_link = |s: &str| {
            ...
            let mut confirmation_link = reqwest::Url::parse(&raw_links).unwrap();
...
pub async fn spawn_app() -> TestApp {
    ...
    TestApp {
        address,
        db_pool: build_connection_pool(&configuration.database),
        email_server,
...
async fn configure_db(config: &DatabaseSettings) -> PgPool {
    ...
    sqlx::migrate!("./migrations")
        .run(&connection_pool)
        .await
        .expect("Failed to mightate the database.");
